    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative z-10">
      <div className="w-full max-w-md p-8 sm:p-10 rounded-[32px] bg-surface-alt/60 backdrop-blur-[24px] backdrop-saturate-[180%] border border-border/50 shadow-2xl">
        ...
        <form className="flex flex-col gap-5 font-[family-name:var(--font-body)]">
          ...
          <div className="flex items-center justify-between mt-[-5px] text-sm">
            <label className="flex items-center gap-2 text-text-secondary cursor-pointer">
              <input type="checkbox" className="w-4 h-4 rounded bg-text-primary/10 border border-border/50 accent-brand-accent" />
              Remember me
            </label>
            <Link href="#" className="text-brand-accent hover:text-brand-accent-hover transition-colors">Forgot Password?</Link>
          </div>

          <button type="button" className="w-full h-12 mt-2 bg-gradient-to-r from-brand-accent to-brand-accent-hover hover:opacity-90 text-[#021213] font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(255,192,80,0.3)]">
        ...
        <p className="mt-8 text-center text-text-secondary text-sm font-[family-name:var(--font-body)]">
          Don't have an account? <Link href="/signup" className="text-brand-accent hover:text-brand-accent-hover font-semibold transition-colors">Sign up</Link>
        </p>
      </div>
    </div>
  );
